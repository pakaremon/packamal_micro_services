...
class Helper:
    ...
    def run_package_analysis(package_name, package_version, ecosystem, local_path=None):
        """
        Run package analysis using the Python analysis runner module.
        
        This method replaces the shell script execution with a pure Python implementation.
        """
        from .analysis_runner import run_package_analysis as runner_run_analysis
        
        print(f" Run package-analysis: Package Name: {package_name}, Package Version: {package_version}, Ecosystem: {ecosystem}")
        
        try:
            # Use the analysis runner module to execute the analysis
            ...
            # Generate report from the JSON data
            ...
            error_output = e.output if hasattr(e, 'output') else (e.stderr if hasattr(e, 'stderr') else str(e))
            error_details = {
                'error_type': 'analysis_execution_failed',
                'exit_code': e.returncode,
                'command': e.cmd if hasattr(e, 'cmd') else 'docker analysis command',
                'stderr': error_output,
            ...
            if e.returncode == 1:
                if 'docker' in error_str and ('not found' in error_str or 'pull' in error_str):
                    error_details['error_category'] = 'docker_image_error'
                    error_details['error_message'] = f"Docker image error: {error_output}"
                elif 'timeout' in error_str:
                    error_details['error_category'] = 'timeout_error'
                    error_details['error_message'] = f"Analysis timeout: {error_output}"
                elif 'permission' in error_str or 'access' in error_str:
                    error_details['error_category'] = 'permission_error'
                    error_details['error_message'] = f"Permission error: {error_output}"
                else:
                    error_details['error_category'] = 'analysis_error'
                    error_details['error_message'] = f"Analysis failed: {error_output}"
            ...
            # Handle JSON parsing errors and other value errors
            error_details = {
                'error_type': 'json_parsing_failed' if isinstance(e, json.JSONDecodeError) else 'value_error',
